# Qt 5 Multimedia for OS/2

This is a port of the `QtMultimedia` module of the [Qt software development framework version 5](https://www.qt.io/) to the OS/2 operating system (and its derivants). This port is carefully crafted and maintained by [bww bitwise works GmbH](https://www.bitwiseworks.com/) (also referred to as *bitwiseworks*).
...
```
yum install qt5-qtmultimedia-examples
```

If you have ArcaOS and prefer a GUI installer, then you should launch *Arca Noae Package Manager* located in the `Computer` > `Install/Remove` folder on your desktop, select `YUM` > `Quick install..` from the main menu, type `qt5-qtmultimedia-examples` in the entry field and press `Enter`.

Once successfully installed, all Qt 5 Multimedia example applications may be found in the `Programs` > `bitwiseworks Apps and Ports` > `Qt 5 Examples` folder on your desktop.
...
### Using Qt 5 Multimedia libraries in your applications

Same as above, using binary builds provided by bitwiseworks is the easiest and the only supported way. Run the following command from the command line prompt to install Qt 5 Multimedia development packages necessary to compile and build Qt 5 programs that use the `QtMultimedia` module:

```
yum install qt5-qtmultimedia-devel
```
...
### Building Qt 5 Multimedia on OS/2

All source code for the OS/2 port of the `QtMultimedia` module is located in our [github repository](https://github.com/bitwiseworks/qtmultimedia-os2) for this module. If you want to build the Qt 5 libraries yourself, please follow our [github instructions](https://github.com/bitwiseworks/qt5-os2/wiki/Developers#building-qt).
